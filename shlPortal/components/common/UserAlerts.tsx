...
import { Seasons, UpdateEvents } from '../../typings';
...
export const UserAlerts = () => {
  ...
  const { season, startDate, ended, loading: seasonLoading } = useSeason();
  ...
  useEffect(
    () => storeSeasonAlert({ season, startDate, ended }),
    [season, startDate, ended],
  );
            ...
            alert.newValue === 'active' ? (
              <UnorderedList>
                ...
                <ListItem>
                  <Link
                    href={
                      player?.draftSeason === season + 1
                        ? process.env.primaryRookieDiscordUrl
                        : process.env.secondaryRookieDiscordUrl
                    }
                  >
